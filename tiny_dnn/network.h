...
class network {
 public:
  ...
  std::vector<vec_t> test(const std::vector<vec_t> &in) {
    ...
    // my test function to test and generate confusion-matrix for 2-classification task
    // labels are length-1 vec_t, predicted labels are value from sigmoid
    result mytest(const std::vector<vec_t> &in, const std::vector<vec_t> &t) {
        result test_result;
        set_netphase(net_phase::test);
        for (size_t i = 0; i < in.size(); i++) {
            vec_t pred_vec = predict(in[i]);
            const label_t predicted = *(pred_vec.begin()) > 0.5 ? 1 : 0;
            const label_t actual    = *(t[i].begin());
