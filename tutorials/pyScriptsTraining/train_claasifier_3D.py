...
with open('/Users/jiazhengsun/Desktop/nn-contact/data/NN-contact-force/train_data/3D/rect_c2_3D_sym.csv') as csvfile:
	...
	for row in shuffle_list:
		...
		y_temp.append(data_digit_str[list_size - 2]) #The last element is always classfier result. The very last char is ","--see trainData script
...
# x_total[:,:3] = x_total[:,:3]/3.14;
# x_total[:,7] = x_total[:,7]/6;
...
#model.add(Dense(64, activation='tanh'))  Graph indicates the performance is about the same when adding an extra layer
...
history = model.fit(x_train, y_train, validation_split = 0.2, epochs=200, batch_size=500, shuffle = True)
